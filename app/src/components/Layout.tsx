...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  const sidebarContent = (
    <div className="flex h-full flex-col">
      {/* Brand */}
      <div className="flex items-center gap-3 px-4 py-5 border-b border-[#2a3a5c]">
        ...
        <div className="flex items-center gap-3">
          ...
          {!collapsed && (
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[#8b9bb4] text-[11px] capitalize">{user?.role || "User"}</p>
            </div>
          )}
        </div>
      </div>
  ...
  return (
    <div className="flex h-screen bg-[#f5f0e8] overflow-hidden">
      {/* Desktop Sidebar */}
      <aside
        className={`hidden lg:flex flex-col bg-[#1e2a4a] border-r border-[#2a3a5c] transition-all duration-200 flex-shrink-0 ${
          collapsed ? "w-16" : "w-64"
        }`}
      >
        {sidebarContent}
        <button
          ...
          className="absolute top-4 -right-3 w-6 h-6 bg-[#1e2a4a] border border-[#2a3a5c] rounded-full flex items-center justify-center text-[#8b9bb4] hover:text-white z-10"
        >
